...
{
    public partial class frmSpreadShirts : XtraForm
    {
        ...
        private int executeLogin(string username, string password)
        {
            try
            {
                ...
                {
                    ApplicationLibary.writeLogThread(lsBoxLog, "[Login]- \"" + username + "\": Username or Password is wrong!", 2);
                    return 0;
                }
                 ...
                 * 
                /*-------------------LOGIN FINISH-------------------*/
                ApplicationLibary.writeLogThread(lsBoxLog, "[LOGIN]- \"" + username + "\" to spread successfully. Start upload...", 1);
                return 1;
            }
            catch (Exception ex)
            {
                ApplicationLibary.writeLogThread(lsBoxLog, "[LOGIN]- Error: " + ex.Message, 2);
                return 0;
            }
        }
        ...
        private void simpleButton1_Click(object sender, EventArgs e)
        {
            ...
            int rs = executeLogin(username, password);
        }
        ...
        private void btnChooesImage_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (DialogResult.OK == op.ShowDialog())
                {
                    ...
                    ApplicationLibary.writeLog(lsBoxLog, "You Selected " + lsImageFileNames.Count + " / 50 file(s)", 1);
                }
            }
            catch (Exception ex)
            {
        ...
        private void enableB(bool b)
        {
            ...
            btnClearAccount.Enabled = b;
            btnImportAccount.Enabled = b;
        }
        ...
        private void btnStart_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                if (listAccountUpload == null || listAccountUpload.Count < 1)
                {
                    XtraMessageBox.Show("Please import list account login to spread!", "Message");
                    enableB(true);
                    return;
                }
        ...
        private void btnImportAccount_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    txtPath.Text = op.FileName;
                    listAccountUpload = new List<OAccount>();

                    using (StreamReader sr = File.OpenText(op.FileName))
                    {
                        string s = String.Empty;
                        while ((s = sr.ReadLine()) != null)
                        {
                            var data = s.Split('|');
                            OAccount oAcc = new OAccount();
                            oAcc.Username = data[0];
                            oAcc.Password = data[1];
                            listAccountUpload.Add(oAcc);
                    ...
                    ApplicationLibary.writeLog(lsBoxLog, "Load data account success: " + listAccountUpload.Count + " record(s)", 1);
                }
            }
            catch (Exception ex)
            {
